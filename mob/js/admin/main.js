...
function getUserGPSSet() {
	...
	mui.ajax(app.getCallDomain() + "/utils/GetGpsSetByGuid", {
		data: {
			userguid: app.getSettings().userGuid
		},
		type: "post",
		dataType: 'json',
		timeout: 5000,
		...
		success: function (data) {
			...
			if (total > 0) {
				...
				//设置签到、签退的菜单显示
				mui.fire(menu, "menu:sign", {
					type: userGPSSet.c_status == "1" || userGPSSet.c_status == "8"
				});
		...
		app.getPosBaidu(function (lat, lng) {
			if (lat > 0) {
				...
				mui.ajax(app.getCallDomain() + "/utils/UserLocationAdd", {
					data: {
						c_user: userGPSSet.c_user,
						c_lat: gps.lat,
						c_lng: gps.lng,
						c_status: (type == true ? 7 : 8)
					}, //7:签到
					type: "post",
					dataType: 'json',
					timeout: 5000,

	...
	mui.ajax(app.getDomain() + get_url, {
		...
		success: function(data) {
			if (data.statusCode == 200) {
				var content = document.body.querySelector('.erp-commpage-stub');
...
//列表item点击事件
mui('.erp-commpage-stub').on('tap', 'a', function(e) {
	var elem = this;
	var url = this.getAttribute('href');
	cur_stub = this.getAttribute('data-id');
	if (url == null) {
		return;
	}else if(url == "/html/commpage/commpage-edit.html"){
		extras = {
		...
		setTimeout(function() {
			mui.openWindow({id: url,	url: url,		extras: extras,		show: {	autoShow: false	}	});
		}, 100);
		return;
	}

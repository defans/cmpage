...
export default class extends CMPage {
    ...
    async pageEditInit(){
        ...
        let task = await cmpage.model('flow/task').getTask(parmsUrl.taskID);
    ...
    async pageSave(parms){
        ...
        if(this.rec.id >0){
            ...
            if(linkModel){
                ...
                await cmpage.model('flow/act').fwRun(parmsUrl.taskActID, this.mod.user, null,true);
            }
        }
    }
    ...
    async addStatus(taskAct,act,user){
        let md ={};
        let task = await cmpage.model('flow/task').getTask(taskAct.c_task);

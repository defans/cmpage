...
module.exports = class extends Base {
  ...
  async saveAction() {
    ...
    if (parms.id == 0) {
      let rec = await model.where({
        c_modulename: parms.c_modulename
      }).find();
  ...
  async editAction() {
    ...
    if (this.get('id') == '0') {
      ...
      Object.assign(md, {
        ...
        proc_name: '',
        c_path: 'cmpage/page',
        c_alias: md.c_modulename,
        c_conn_name: 'admin',
        c_pk: 'id'
      });
    } else {
      let tmp = await this.model("t_module", 'cmpage').where({
        id: this.get('id')
      }).find();
  ...
  async col_listAction() {
    ...
    vb.editList = await this.model('t_module_col', 'cmpage').where({
      c_module: this.get('moduleid')
    }).order('c_order ').select();
    ...
    for (let i = 0; i < 100; i++) {
      ...
      if (!think.isEmpty(posts[`editList[${i}].c_column`])) {
        ...
        if (think.isEmpty(posts[`editList[${i}].id`])) {
          ...
          await model.where({
            id: parseInt(posts[`editList[${i}].id`])
          }).update(cmpage.checksql(md));
        }
  ...
  async edit_listAction() {
    ...
    vb.editList = await this.model('t_module_edit', 'cmpage').where({
      c_module: this.get('moduleid')
    }).order('c_order ').select();
  ...
  async edit_saveAction() {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      if (!think.isEmpty(posts[`editList[${i}].c_column`])) {
        ...
        md.c_isshow = true; // !think.isEmpty(posts[`editList[${i}].c_isshow`]);
  ...
  async query_listAction() {
    ...
    vb.editList = await this.model('t_module_query', 'cmpage').where({
      c_module: this.get('moduleid')
    }).order('c_order ').select();
  ...
  async btn_listAction() {
    ...
    let vb = {};
    vb.editList = await this.model('t_module_btn', 'cmpage').where({
      c_module: this.get('moduleid')
    }).order('c_location ').select();

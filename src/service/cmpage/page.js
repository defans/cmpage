...
module.exports = class extends PageBase {
                    ...
                    if (item[this.pk] !== 0) {
                        if (!think.isEmpty(col.c_format)) {
                            if (col.c_coltype === "float") {
                                html.push(cmpage.formatNumber(Number(item[col.c_column]), {
                                    pattern: col.c_format
                                }));
    ...
    async htmlGetEdit() {
        ...
        for (let col of this.modEdits) {
            if (!col.c_editable || col.c_column === this.pk) {
                continue;
            }
            ...
            if (col.c_type === "datetime" || col.c_type === "date") {
                ...
                let options = {
                    ...
                    formData: {
                        link_type: this.mod.c_table,
                        inputName: col.c_column
                    },
    ...
    async htmlGetView(isPrintStyle) {
        ...
        for (let col of this.modCols) {
            ...
            if (col.c_type === "checkbox") {
                ...
                html.push(cmpage.formatNumber(Number(md[col.c_column]), {
                    pattern: col.c_format
                }));

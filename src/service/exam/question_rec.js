...
module.exports = class extends CMPage {
    ...
    async pageSave(parms){
        ...
        if(parms.c_is_answer){
            let question = await this.model('t_question').where({id:parms.c_question}).find();
        ...
        if(ret.statusCode == 200 ){
            //如果是多选题，重置试题的答案项
            let recs = await this.model('t_question_rec').where({c_question:parms.c_question}).select();
            let no = [];
            for(let rec of recs){

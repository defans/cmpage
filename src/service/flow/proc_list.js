...
module.exports = class extends CMPage {
    ...
    async getFlowMap(procID) {
        let md = await this.model('fw_proc').where({
            id: procID
        }).find();
        ...
        if (think.isObject(flowMap)) {
            ...
            for (let k in flowMap.states) {
                if (flowMap.states[k].data_id == 0) {
                    ...
                    think.log(rect);
                    let act = {
                        c_name: rect.text.text,
                        c_desc: rect.text.text,
                        c_proc: parms.procID,
                        c_type: 1,
                        c_class: 'flow/act',
                        c_btn_style: '',
            ...
            for (let k in flowMap.paths) {
                ...
                let fromAct = await this.model('fw_act').where({
                    c_proc: parms.procID,
                    c_map_id: path.from
                }).find();
                let toAct = await this.model('fw_act').where({
                    c_proc: parms.procID,
                    c_map_id: path.to
                }).find();
                if (!think.isEmpty(fromAct) && !think.isEmpty(toAct)) {
                    let rec = {
                        c_name: path.text.text,
                        c_proc: parms.procID,
                        c_from: fromAct.id,
                        c_to: toAct.id,
                        c_status: 0
            ...
            let actStart = await this.model('fw_act').where({
                c_proc: parms.procID,
                c_type: cmpage.enumActType.START
            }).find();
            await this.model('fw_proc').where({
                id: parms.procID
            }).update({
                c_map: map,
                c_act_start: think.isEmpty(actStart) ? 0 : actStart.id
            });
        }
    }
    ...
    async copyToNewProc(procID) {
        ...
        if (think.isEmpty(proc)) return {
            statusCode: 300,
            message: '源流程模板不存在!',
            proc: {}
        };
        let actList = await this.model('fw_act').where({
            c_proc: procID
        }).select();
        if (think.isEmpty(actList)) return {
            statusCode: 300,
            message: '源流程模板未设置流程节点!',
            proc: {}
        };
        ...
        await this.model('fw_proc').where({
            ...
            c_map: cmpage.objToString(flowMap),
            c_act_start: proc.c_act_start
        });
        return {
            statusCode: 200,
            message: '复制成功! 新的流程模板：' + proc.c_name,
            proc: proc
        };
    }

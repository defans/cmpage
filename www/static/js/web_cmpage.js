...
function pageSaveByAct(modulename,reloadUrl,flag){
            ...
            BJUI.ajax('ajaxform', {
                ...
                okCallback: function (json, options) {
                    if (flag === 'close') {
                        ...
                        var rec = json.data;
                        BJUI.dialog('reload', {url: reloadUrl +'&id=' + rec.id});
                    }
                }
            });
        }
...
// function pageSaveByAct(modulename,procID,actID,status,linkID,linkType,linkModel,flag){
//     //var page = pageGetCurrent(obj);
//     BJUI.alertmsg("confirm", "是否确定要执行本操作？", {
//         okCall: function () {
//             BJUI.ajax('ajaxform', {
...
//                     } else {
//                         var rec = json.data;
//                         var url = '/cmpage/page/edit?modulename=' + modulename + '&id=' + rec.id + '&procID=' + procID + '&actID='
//                             + actID + '&status=' + status + '&linkID=' + linkID + '&linkType=' + linkType + '&linkModel=' + linkModel + '&listIds=';
...
function pageSaveMs(modulename,reloadUrl,editID,pk){
    ...
    BJUI.ajax('ajaxform', {
        ...
        okCallback: function (json, options) {
            if(editID === 0){
                var rec = json.data;
                //var url = '/cmpage/page/edit_ms?modulename=' + modulename + '&'+pk+'=' + rec.id  + '&listIds='+ '&parmsUrl=' + parmsUrl;
...
// /* cmpage/utils/call_function 根据URL参数直接调用相关model的相关方法,  本方法的参数必填 */
// -----------TODO: 如果需要进一步执行什么动作，可以考虑在这里扩展， 目前采用的方式： <button data-toggle="doajax" ....
// function utilsCallFunction(model,fn,parms){
//     BJUI.ajax('doajax', {
//         url: '/cmpage/utils/call_function?model='+model+'&fn='+fn+'&parms='+parms,
//         loadingmask: true,
//         okCallback: function (json, options) {
//             if(json.statusCode == 200){
//                 if(json.messag){    BJUI.alertmsg("warn", "下载文件失败！");   }
//             }else{
//                 BJUI.alertmsg("warn", json.messag);
//             }
//         }

...
+function ($) {
        ...
        var tools = {
            ...
            initTbody: function () {
                ...
                $tbody.find('> tr').each(function (index) {
                    ...
                    //lyf--begin  每行之前增加id的隐藏字段，以便全部保存的时候ID值能往后端递交
                    if ($tr.attr('data-id')) {
                        var idname = $table.find('> thead > tr:eq(0)').data('idname') || 'id';
                        $(this).before('<input type="hidden" name="' + idname.replaceSuffix(index) + '" value="' + $tr.attr('data-id') + '">');
                    }
                    //lyf--end
                    ...
                    //lyf--: 增加一个属性 edit-always 表示该行保持编辑状态
                    if ($tr.attr('data-edit-always') != "true") {
                        _doRead($tr)
                    }
                })
    ...
    Tabledit.prototype.add = function ($table, num) {
        ...
        for (var i = 0; i < num; i++) {
            //lyf-- 下一句增加 style="' + $(this).attr("style") + '" ，以便增加的行可以和th设置的style一致，如隐藏列等
            $tr.find('> th').each(function () { $(this).replaceWith('<td style="' + $(this).attr("style") + '">' + $(this).html() + '</td>') })
